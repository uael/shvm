...
int main(int ac, char *av[], char *ev[])
{
	...
	// sleep 1
	*shvm_oppush(NULL) = (t_op){OP_CLR,		OPF_FULL,			0,	0,	0}; 	// clear the context (full)
	*shvm_oppush(NULL) = (t_op){OP_BIN,		OPF_LOCK,			6,	5,	0}; 	// "sleep" (locked because no quote/glob/var)
	...
	*shvm_oppush(NULL) = (t_op){OP_COMP,	0,					0,	0,	0}; 	// if 0 continue else jump

	// cat /dev/random |
	*shvm_oppush(NULL) = (t_op){OP_CLR,		OPF_FULL,			0,	0,	0}; 	// clear the context (full)
	*shvm_oppush(NULL) = (t_op){OP_BIN,		OPF_LOCK,			18,	3,	0}; 	// "cat" (locked because no quote/glob/var)
	...
	*shvm_oppush(NULL) = (t_op){OP_EXEC,	0,					0,	0,	0}; 	// exec
	...
	// base64 |
	*shvm_oppush(NULL) = (t_op){OP_CLR,		0,					0,	0,	0}; 	// clear the context (only argv/envv)
	*shvm_oppush(NULL) = (t_op){OP_BIN,		OPF_LOCK,			36,	6,	0}; 	// "base64" (locked because no quote/glob/var)
	*shvm_oppush(NULL) = (t_op){OP_FORK,	OPF_PIPE,			0,	0,	0}; 	// fork
	*shvm_oppush(NULL) = (t_op){OP_EXEC,	0,					0,	0,	0}; 	// exec

	// head -c 8
	*shvm_oppush(NULL) = (t_op){OP_CLR,		0,					0,	0,	0}; 	// clear the context (only argv/envv)
	*shvm_oppush(NULL) = (t_op){OP_BIN,		OPF_LOCK,			45,	4,	0}; 	// "head" (locked because no quote/glob/var)
	*shvm_oppush(NULL) = (t_op){OP_FORK,	0,					0,	0,	0}; 	// fork
